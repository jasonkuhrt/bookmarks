...
describe("withOrchestratedWorkspacePublish", () => {
  test("replays queued workspace publish targets on the next successful run", async () => {
    const runtimeDir = await mkdtemp(join(tmpdir(), "bookmarks-runtime-"));
    process.env["BOOKMARKS_RUNTIME_DIR"] = runtimeDir;

    await writeFile(
      Paths.defaultWorkspacePublishQueuePath(),
      JSON.stringify({
        operation: "publish",
        workspacePath: "/tmp/workspace.yaml",
        requestedTargetIds: ["safari"],
        blockers: ["Safari"],
        queuedAt: "2026-01-01T00:00:00.000Z",
      }),
    );

    let executedTargets: readonly string[] | undefined;

    const result = await run(
      Orchestration.withOrchestratedWorkspacePublish(
        "/tmp/workspace.yaml",
        ["chrome/default"],
        (requestedTargetIds) => {
          executedTargets = requestedTargetIds;
          return Effect.succeed("done");
        },
      ),
    );
    ...
    expect(result).toEqual({ _tag: "completed", value: "done" });
    expect(executedTargets).toEqual(["safari", "chrome/default"]);
    await expect(access(Paths.defaultWorkspacePublishQueuePath())).rejects.toThrow();
  });

  test("queues workspace publish when browser blockers are encountered", async () => {
    const runtimeDir = await mkdtemp(join(tmpdir(), "bookmarks-runtime-"));
    process.env["BOOKMARKS_RUNTIME_DIR"] = runtimeDir;

    const result = await run(
      Orchestration.withOrchestratedWorkspacePublish("/tmp/workspace.yaml", ["safari"], () =>
        Effect.fail(new Orchestration.TemporarySyncBlocker({ blockers: ["Safari"] })),
      ),
    );
    ...
    if (result._tag === "deferred") {
      expect(result.notice.state).toBe("queued");
      expect(result.notice.operation).toBe("publish");
      expect(result.notice.message).toContain("queued");
      expect(result.notice.blockers).toEqual(["Safari"]);
    }

    const queued = JSON.parse(
      await readFile(Paths.defaultWorkspacePublishQueuePath(), "utf-8"),
    ) as {
      readonly operation: string;
      readonly requestedTargetIds: readonly string[];
      readonly blockers: readonly string[];
    };
    expect(queued.operation).toBe("publish");
    expect(queued.requestedTargetIds).toEqual(["safari"]);
    expect(queued.blockers).toEqual(["Safari"]);
  });
});
