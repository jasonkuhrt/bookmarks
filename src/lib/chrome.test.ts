...
describe("importBookmarks", () => {
  test("imports folders, separators, and raw nodes into workspace review state", async () => {
    const dir = await mkdtemp(join(tmpdir(), "bookmarks-chrome-import-"));
    const path = join(dir, "Bookmarks");
    ...
    try {
      ...
      const snapshot = await run(Chrome.importBookmarks(path, "chrome/default"));
      const bar = snapshot.tree.bar ?? [];
      expect(bar[0]).toMatchObject({ kind: "folder", title: "Folder" });
      expect(bar[1]).toMatchObject({ kind: "separator" });
      expect(bar[2]).toMatchObject({ kind: "raw", title: "Mystery" });
      expect(snapshot.occurrences.map((occurrence) => occurrence.kind)).toEqual([
        "folder",
        "bookmark",
        "separator",
        "raw",
