  ...
  const profiles = isRecord(value["profiles"])
    ? Object.fromEntries(
        Object.entries(value["profiles"])
          .map(([profile, profileValue]) => [profile, normalizeChromeProfileDocument(profileValue)] as const)
...
const ensureChromeTreeDoesNotUseReadingList = (
  ...
  if (tree?.["reading_list"] !== undefined) {
    throw new Error(`${label} cannot define reading_list. Use top-level all.reading_list or safari.reading_list instead.`)
  }
}

const deriveBrowserEnabled = (profiles: Record<string, unknown> | undefined): boolean | undefined => {
  if (!profiles || Object.keys(profiles).length === 0) return undefined
  const enabledStates = Object.values(profiles)
    .map((value) => isRecord(value) && typeof value["enabled"] === "boolean" ? value["enabled"] : true)
...
const normalizeLegacyDocument = (value: unknown): unknown => {
  ...
  if (chromeTargets) {
    for (const [profile, targetValue] of Object.entries(chromeTargets)) {
      ...
      chromeProfiles[profile] = "enabled" in targetValue
        ? { enabled: targetValue["enabled"] }
        : {}
    }
  }
...
const normalizeDocument = (value: unknown): unknown => {
  ...
  const normalized = {
    ...
    ...(legacy["safari"] !== undefined ? { safari: normalizeSafariDocument(legacy["safari"]) } : {}),
    ...(legacy["chrome"] !== undefined ? { chrome: normalizeChromeDocument(legacy["chrome"]) } : {}),
  }
...
export const save = (path: string, config: BookmarksConfig): Effect.Effect<void, Error> =>
  Effect.gen(function* () {
    const encoded = yield* Schema.encode(BookmarksConfig)(BookmarksConfig.make({
      version: 2,
      all: config.all,
      safari: config.safari ? SafariBookmarks.make(config.safari) : undefined,
      chrome: config.chrome ? ChromeBookmarks.make({
        ...config.chrome,
...
export const isTargetEnabled = (
  ...
  switch (target.browser) {
    ...
    case "chrome":
      return (config.chrome?.enabled ?? true)
    ...
    case "chrome":
      return Effect.succeed(
        mergeTrees(
          ...
          projectTreeForBrowser(treeFrom(target.profile ? config.chrome?.profiles?.[target.profile] : undefined), "chrome"),
        ),
      )
    default:
