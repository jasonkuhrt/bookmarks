/* oxlint-disable no-non-null-assertion, no-shadow, no-unsafe-type-assertion, no-unnecessary-condition, prefer-await-to-then, restrict-template-expressions */
...
const countNodes = (tree: WorkspaceTree): number => {
  const countSection = (nodes: readonly WorkspaceNode[] | undefined): number =>
    (nodes ?? []).reduce(
      (total, node) => total + 1 + (node.kind === "folder" ? countSection(node.children) : 0),
      0,
    );
...
const workspaceHash = (raw: string): string => createHash("sha256").update(raw).digest("hex");
...
export const loadImportLock = (
  path = Paths.defaultImportLockPath(),
): Effect.Effect<ImportLock, Error> => readJsonFile<ImportLock>(path);

const saveImportLock = (path: string, value: ImportLock): Effect.Effect<void, Error> =>
  writeJsonFile(path, value);

export const loadPlan = (
  path = Paths.defaultPublishPlanPath(),
): Effect.Effect<WorkspacePlan, Error> => readJsonFile<WorkspacePlan>(path);
...
const emptyYamlConfig = (): BookmarksConfig => BookmarksConfig.make({ all: BookmarkTree.make({}) });

const resolveTargets = (
  requestedTargetIds: readonly string[],
): Effect.Effect<Readonly<Record<string, WorkspaceTarget>>, Error> =>
  Effect.gen(function* () {
    ...
    const explicitlyRequestedDisabled = resolvedDescriptors.filter(
      (descriptor) =>
...
const walkTree = (
  ...
  const walkSection = (
    nodes: readonly WorkspaceNode[] | undefined,
...
const walkScopedTrees = (
  ...
  walkTree(scopedTrees.global, (node, path) => {
    visit(node, ["global", ...path]);
  });
  for (const [targetId, tree] of Object.entries(scopedTrees.profiles)) {
    walkTree(tree, (node, path) => {
      visit(node, ["profiles", targetId, ...path]);
    });
  }
};
...
const validateAgainstImportLock = (
  ...
  const allOccurrences = new Set(
    Object.values(importLock.targets).flatMap((target) =>
      target.occurrences.map((occurrence) => occurrence.id),
    ),
  );
    ...
    walkTree(tree, (node, path) => {
      ...
      for (const source of node.sources ?? []) {
        if (!allOccurrences.has(source)) {
          errors.push(
            `Unknown source occurrence ${source} referenced at ${label}/${path.join("/")}`,
          );
        }
      }
    });
...
const mergeBookmarkTrees = (base: BookmarkTree, overlay: BookmarkTree): BookmarkTree =>
  BookmarkTree.make({
    ...
    reading_list: nonEmptyOrUndefined([
      ...(base.reading_list ?? []),
      ...(overlay.reading_list ?? []),
    ]),
...
const convertTreeForPublish = (
  ...
  readonly seenUrls: ReadonlyMap<string, string>;
} => {
  const blockers: WorkspacePlanBlocker[] = [];
  ...
  const bookmarkTree = BookmarkTree.make({
    ...
    reading_list: nonEmptyOrUndefined(
      convertSection(tree.reading_list, [...locationRoot, "reading_list"]),
    ),
...
const buildPlan = (
  ...
  { readonly plan: WorkspacePlan; readonly publishTrees: Readonly<Record<string, BookmarkTree>> },
  Error
> =>
  Effect.gen(function* () {
    const blockers: WorkspacePlanBlocker[] = [];
...
export const plan = (): Effect.Effect<WorkspacePlan, Error> => planFor([]);

export const planFor = (
  requestedTargetIds: readonly string[],
): Effect.Effect<WorkspacePlan, Error> =>
  Effect.gen(function* () {
...
const plansEqual = (left: BookmarkTree, right: BookmarkTree): Effect.Effect<boolean, Error> =>
  Effect.gen(function* () {
...
export const publish = (): Effect.Effect<WorkspacePublishResult, Error> => publishTo([]);

export const publishTo = (
  requestedTargetIds: readonly string[],
): Effect.Effect<WorkspacePublishResult, Error> =>
  Effect.gen(function* () {
    const { workspacePath, importLockPath, planPath } = workspaceFiles();
    const rawWorkspace = yield* Effect.tryPromise({
      try: () => Fs.readFile(workspacePath, "utf-8"),
  ...
  Effect.gen(function* () {
    ...
    if (!validation.valid) {
      return {
        ...
        nextAction: editFileAction(
          workspacePath,
          "Fix invalid workspace references before planning a publish.",
        ),
      };
    }
    ...
    if (summaryBase.inboxItems > 0 || summaryBase.quarantineItems > 0) {
      return {
        ...
        nextAction: editFileAction(
          workspacePath,
          "Review inbox and quarantine items in workspace.yaml.",
        ),
      };
    }
    ...
    if (built.plan.blockers.length > 0) {
      const requiresWorkspaceEdit = built.plan.blockers.some(
        (blocker) =>
          blocker.code === "duplicate-url" ||
          blocker.code === "unsupported-node" ||
          blocker.code === "review-inbox" ||
      ...
      return {
        ...
        nextAction: requiresWorkspaceEdit
          ...
          : runCommandAction(
              "bookmarks plan --json",
              "Resolve environment blockers before publishing.",
            ),
      };
    }
    ...
    if (!savedPlan || savedPlan.workspaceHash !== built.plan.workspaceHash) {
      return {
        ...
        nextAction: runCommandAction(
          "bookmarks plan",
          "Generate a fresh publish plan for the current workspace.",
        ),
      };
    }
    ...
    if (savedPlan.publishedAt) {
      return {
        ...
        summary: built.plan.summary,
        blockers: [],
        nextAction: doneAction(
          "Workspace has been published. Re-import when you want to refresh from browsers.",
        ),
      };
    }
    ...
    return {
      ...
      nextAction: runCommandAction(
        "bookmarks publish",
        "Publish the curated workspace back to configured targets.",
      ),
    };
  });
