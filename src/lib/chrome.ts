...
const nowChromeTimestamp = (): string =>
  unixMsToChromeTimestamp(DateTime.toEpochMillis(DateTime.unsafeNow()));
...
export const importBookmarks = (
  ...
  Effect.gen(function* () {
    ...
    const importNodes = (children: ChromeNode[], parentPath: readonly string[]): WorkspaceNode[] =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.tryPromise({
      try: async () => {
        ...
        new Error(`Failed to write Chrome bookmarks at ${bookmarksPath}`, { cause }),
    });
  });

export const writeTree = (bookmarksPath: string, tree: BookmarkTree): Effect.Effect<void, Error> =>
  Effect.gen(function* () {
    const text = yield* Effect.tryPromise({
      try: () => Bun.file(bookmarksPath).text(),
      catch: (cause) => new Error(`Failed to read Chrome bookmarks at ${bookmarksPath}`, { cause }),
    ...
    roots.bookmark_bar.children = buildChromeChildren(tree.bar, "bar", existing, () => nextId++);
    roots.other.children = buildChromeChildren(tree.menu, "menu", existing, () => nextId++);
    roots.synced.children = buildChromeChildren(tree.mobile, "mobile", existing, () => nextId++);
...
const buildChromeChildren = (
  ...
  (nodes ?? []).map((node) => {
    ...
    folder.children = buildChromeChildren(node.children, folderPath, lookup, allocateId);
