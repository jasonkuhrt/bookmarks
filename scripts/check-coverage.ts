...
if (!lcovPath) {
  console.error("Usage: bun scripts/check-coverage.ts <lcov-path> [min-lines] [min-functions]")
  process.exit(1)
}
...
const functionsPct = average(records.map((record) => percent(record.functionsHit, record.functionsFound)))
